...
class perfiluser : AppCompatActivity() {
    ...
    private val databaseUrl =
        "https://uniforlibrary-30c5c-default-rtdb.firebaseio.com/"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val ivEdit =
            findViewById<ImageView>(R.id.ivEditPhoto)
    ...
    private fun carregarDadosUsuario() {
        ...
        val tvNome =
            findViewById<TextView>(R.id.tvNomeUsuario)

        val tvCentro =
            findViewById<TextView>(R.id.tvCentroUsuario)
        ...
        lifecycleScope.launch {
            ...
            try {
                ...
                val centro =
                    snapshot.child("centro")
                        .getValue(String::class.java)
                        ?: "Não informado"

                val pontos =
                    snapshot.child("pontos")
                        .getValue(Int::class.java)
                        ?: 0

                val fotoUrl =
                    snapshot.child("fotoUrl")
                        .getValue(String::class.java)

                val acessados =
                    snapshot.child("livrosAcessados")
                        .getValue(Int::class.java)
                        ?: 0

                val emEmprestimo =
                    snapshot.child("livrosEmEmprestimo")
                        .getValue(Int::class.java)
                        ?: 0

                val emprestados =
                    snapshot.child("livrosEmprestados")
                        .getValue(Int::class.java)
                        ?: 0

                val avaliacoes =
                    snapshot.child("avaliacoesFeitas")
                        .getValue(Int::class.java)
                        ?: 0

                val total =
                    snapshot.child("totalLivros")
                        .getValue(Int::class.java)
                        ?: 0
